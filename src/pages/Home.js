    ...
    <div className='container mb-5'>
        ...
        {Journey && Journey.filter(x => { 
                  if(query === ""){
                    ...
                    return x
                  }else if(dateFormat(x.createdAt, 'dddd, d mmmm, yyyy').toLowerCase().includes(query.toLowerCase())){
                    return x
                  }
                }).map((x, index)=>
               ...
               <Card.Img variant="top" src={x.image} />
                   <Card.Body>
                       <Card.Title>{x.title.slice(0,5)}</Card.Title>
                       <Card.Text>
                         <div className="post__description" dangerouslySetInnerHTML={{ __html: x.body.slice(0, 15)}}  />
                       </Card.Text>
